...
function activateTab(tabId) {
  ...
  if (tabId === 'HERRAMIENTAS')  loadToolsStatus();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initToolsActions(); // Tab HERRAMIENTAS: funciones huérfanas del backend
...
/* =========================================================================
   TAB HERRAMIENTAS — Funciones huérfanas del backend sin UI previa
   - classifier/classify, downloads/verify, rag/search, neo4j/sync,
     extract/structured, analytics/cached-data
   ========================================================================= */

function initToolsActions() {
  const btnClassify = $('#btn-tools-classify');
  const btnVerify   = $('#btn-tools-verify');
  const btnRag      = $('#btn-tools-rag');
  const btnNeo4j    = $('#btn-tools-neo4j');
  const btnExtract  = $('#btn-tools-extract');
  const btnAnalytics= $('#btn-tools-analytics');

  if (btnClassify) btnClassify.addEventListener('click', runToolsClassify);
  if (btnVerify)   btnVerify.addEventListener('click', runToolsVerify);
  if (btnRag)      btnRag.addEventListener('click', runToolsRag);
  if (btnNeo4j)    btnNeo4j.addEventListener('click', runToolsNeo4j);
  if (btnExtract)  btnExtract.addEventListener('click', runToolsExtract);
  if (btnAnalytics)btnAnalytics.addEventListener('click', loadToolsAnalytics);

  // Enter en los inputs dispara la acción
  const ci = $('#tools-classify-input');
  if (ci) ci.addEventListener('keydown', e => { if (e.key === 'Enter') runToolsClassify(); });
  const ri = $('#tools-rag-input');
  if (ri) ri.addEventListener('keydown', e => { if (e.key === 'Enter') runToolsRag(); });
  const ei = $('#tools-extract-clave');
  if (ei) ei.addEventListener('keydown', e => { if (e.key === 'Enter') runToolsExtract(); });
}

async function loadToolsStatus() {
  try {
    const r = await fetch('/api/downloads/verify-status');
    if (r.ok) {
      const d = await r.json();
      const badge = $('#tools-verify-badge');
      if (badge) {
        badge.textContent = `${d.verified}/${d.total} ok · ${d.corrupt} corruptos (${d.health_pct}%)`;
        badge.style.color = d.corrupt > 0 ? 'var(--color-warn)' : 'var(--color-green)';
      }
    }
  } catch (e) { /* no crítico */ }
}

function runToolsClassify() {
  const input = $('#tools-classify-input')?.value?.trim();
  const out   = $('#tools-classify-out');
  if (!input) { showToast('Escribe una clave o archivo', 'warn'); return; }
  if (out) out.textContent = 'clasificando...';
  fetch(`/api/classifier/classify?input_string=${encodeURIComponent(input)}`)
    .then(r => r.json())
    .then(d => {
      if (out) out.textContent = JSON.stringify(d, null, 2);
      // Colorear badge de fuente con las clases ya existentes (badge--semarnat / badge--asea)
      const src = (d.source || '').toUpperCase();
      const badgeClass = src === 'SEMARNAT' ? 'badge--semarnat' : src === 'ASEA' ? 'badge--asea' : '';
      if (badgeClass && out) {
        const line = document.createElement('div');
        line.className = `badge ${badgeClass}`;
        line.style.marginTop = '6px';
        line.textContent = `FUENTE: ${src}`;
...
function runToolsVerify() {
  const limit   = parseInt($('#tools-verify-limit')?.value || '100', 10);
  const out     = $('#tools-verify-out');
  const progEl  = $('#tools-verify-progress');
  const btn     = $('#btn-tools-verify');
  if (out) out.textContent = 'auditoría en progreso...';
  setProgress(progEl, 0);
  if (btn) btn.disabled = true;

  fetch('/api/downloads/verify-all', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    ...
    .then(d => {
      ...
      const badge = $('#tools-verify-badge');
      if (badge) {
        badge.textContent = `${d.verified}/${d.total_audited} ok · ${d.corrupt} corruptos`;
        badge.style.color = d.corrupt > 0 ? 'var(--color-warn)' : 'var(--color-green)';
      }
      showToast(`Verificación: ${d.verified} ok, ${d.corrupt} corruptos`, d.corrupt > 0 ? 'warn' : 'ok');
    })
    .catch(err => { if (out) out.textContent = `ERROR: ${err.message}`; })
    .finally(() => { if (btn) btn.disabled = false; });
}

function runToolsRag() {
  const q      = $('#tools-rag-input')?.value?.trim();
  const clave  = $('#tools-rag-clave')?.value?.trim() || '';
  const topk   = parseInt($('#tools-rag-topk')?.value || '5', 10);
  const out    = $('#tools-rag-out');
  if (!q) { showToast('Escribe una consulta', 'warn'); return; }
  if (out) out.textContent = 'buscando...';

  let url = `/api/rag/search?query=${encodeURIComponent(q)}&top_k=${topk}`;
  if (clave) url += `&clave=${encodeURIComponent(clave)}`;

  fetch(url)
    .then(r => r.json())
    .then(d => {
      const chunks = (d.chunks || []).slice(0, 8).map(c =>
        `• [${(c.score ?? 0).toFixed(3)}] ${escHtml((c.section_title || c.clave || ''))}\n  ${(c.chunk_text || '').slice(0, 160).replace(/\n/g, ' ')}...`
      ).join('\n\n');
      if (out) out.textContent = chunks || '[ sin resultados ]';
    })
    .catch(err => { if (out) out.textContent = `ERROR: ${err.message}`; });
}

function runToolsNeo4j() {
  const clear  = $('#tools-neo4j-clear')?.checked ? 'true' : 'false';
  const out    = $('#tools-neo4j-out');
  const progEl = $('#tools-neo4j-progress');
  const badge  = $('#tools-neo4j-badge');
  const btn    = $('#btn-tools-neo4j');
  if (out) out.textContent = '';
  setProgress(progEl, 0);
  if (btn) btn.disabled = true;
  if (badge) { badge.textContent = 'SINC...'; badge.style.color = 'var(--color-warn)'; }

  appendLog(out, 'Iniciando carga al Neo4j...', 'info');
  const es = new EventSource(`/api/neo4j/sync?dry_run=true&clear=${clear}`);
  es.onmessage = e => {
    const evt = JSON.parse(e.data);
    if (evt.pct !== undefined) setProgress(progEl, evt.pct);
    appendLog(out, evt.msg || evt.status, evt.status === 'error' ? 'error' : 'info');
    if (evt.status === 'complete' || evt.status === 'error') {
      es.close();
      if (btn) btn.disabled = false;
      if (badge) {
        badge.textContent = evt.status === 'complete' ? 'OK' : 'ERROR';
        badge.style.color = evt.status === 'complete' ? 'var(--color-green)' : 'var(--color-red)';
      }
    }
  };
  es.onerror = () => {
    appendLog(out, 'Conexión SSE perdida (¿driver Neo4j no instalado?)', 'error');
    es.close();
    if (btn) btn.disabled = false;
    if (badge) { badge.textContent = 'OFFLINE'; badge.style.color = 'var(--color-red)'; }
  };
}

function runToolsExtract() {
  const clave = $('#tools-extract-clave')?.value?.trim();
  const out   = $('#tools-extract-out');
  const btn   = $('#btn-tools-extract');
  if (!clave) { showToast('Escribe la clave del proyecto', 'warn'); return; }
  if (out) out.textContent = 'extrayendo (LLM)...';
  if (btn) btn.disabled = true;
  ...
  fetch('/api/extract/structured', {
    ...
    .then(async r => {
      ...
      if (out) out.textContent = JSON.stringify(d, null, 2);
      if (r.ok) showToast(`Extracción estructurada OK: ${clave}`, 'ok');
      else showToast(`Error: ${d.detail || r.status}`, 'error');
    })
    .catch(err => { if (out) out.textContent = `ERROR: ${err.message}`; })
    .finally(() => { if (btn) btn.disabled = false; });
}
...
async function loadToolsAnalytics() {
  const out   = $('#tools-analytics-out');
  const grid  = $('#tools-analytics-grid');
  const badge = $('#tools-analytics-badge');
  if (out) out.textContent = 'cargando...';
  try {
    const r = await fetch('/api/analytics/cached-data');
    const d = await r.json();
    const data = d.data || {};
    if (out) out.textContent = JSON.stringify(data, null, 2);
    if (grid) {
      const keys = ['gacetas_md', 'proyectos_con_inference', 'proyectos_sin_inference', 'total_claves'];
      grid.innerHTML = keys.map(k =>
        `<div style="background:rgba(0,0,0,0.3); border:1px solid var(--border-color-dim); padding:8px; text-align:center;">
           <div style="font-family:var(--font-mono); font-size:16px; color:var(--color-amber); font-weight:bold;">${data[k] ?? '─'}</div>
           <div class="text-xs text-muted">${k.replace(/_/g, ' ')}</div>
         </div>`
      ).join('');
    }
    if (badge) {
      badge.textContent = data.ready_for_neo4j ? 'listo' : 'pendiente';
      badge.style.color = data.ready_for_neo4j ? 'var(--color-green)' : 'var(--color-warn)';
    }
  } catch (err) {
